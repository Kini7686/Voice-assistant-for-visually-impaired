...
def sendEmail(to, content):
    ...
    server.login('adityakini7686@gmail.com', 'Aditya@7686')
    server.sendmail('adityakini7686@gmail.com', to, content)
    server.close()
...
def respond(voice_data):
    ...
    if there_exists(["search", "meaning for "])  and 'youtube' not in voice_data:
        #search_term = voice_data.replace("search","")
        #url = "https://google.com/search?q=" + search_term
        #webbrowser.get().open(url)
        #engine_speak("Here is what I found for" + search_term + "on google")
        person=voice_data.replace('search for','')
    ...
    if there_exists(["capture","my screen","screenshot"]):
        ...
        myScreenshot.save('E:/Candice/Screenshot/screen.png')
   ...
   # Current location as per Google maps
    ...
    if there_exists(['email to gaurav']) :
            try:
                engine_speak("What should I say?")
                content = record_audio()
                to = "samatacablenetwork@gmail.com"   
    ...
    if there_exists(['send a mail']) :
            try:
                engine_speak("What should I say?")
                content = record_audio()
                engine_speak("whome should i send")
                to = input()   
                sendEmail(to, content)
                engine_speak("Email has been sent !")
            except Exception as e:
